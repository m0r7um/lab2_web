...
function drawTopRightQuadrant() {
    ctx.beginPath();
    ctx.fillRect(maxLength / 2 - (R / 2), maxLength / 2, R/2, R);
    ctx.stroke();
}

function drawDownLeftQuadrant() {
    ctx.beginPath();
    ctx.moveTo(maxLength / 2, maxLength / 2 - R / 2);
    ctx.lineTo(maxLength / 2, maxLength / 2);
    ctx.lineTo(maxLength / 2 + R / 2, maxLength / 2);
    ctx.closePath();
    ctx.fill();
}
...
function drawPoint(x, y, r) {
    ctx.fillRect(maxLength / 2 + R * (x / r) - 1,maxLength / 2 - R * (y / r) - 1,4,4);
}
...
canvas.onclick = (event) => {
    if (validR()) {
        ...
        document.getElementById(String(Math.round(x / R * userR))).checked = true;
        if (isValid()) {
